//! `telos-prove` — standalone contradiction-checker for named constraint groups.
//!
//! Reads named constraint groups as JSON (from a file argument or stdin) and
//! reports pairwise/joint contradictions via the self-contained FM solver. For
//! callers with no `.telos` source who want a standalone binary rather than a
//! library dependency.
//!
//! Usage:
...
use crate::prove::{build_report, parse_groups};
