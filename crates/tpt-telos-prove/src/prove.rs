//! Constraint-group schema, contradiction report, and output formatters.
//!
//! Input is JSON describing one or more named constraint *groups*. Each group
//! is a set of linear-arithmetic [`Constraint`]s. We then:
//!
//! * detect any group that is **self-unsatisfiable** on its own, and
//! * reuse [`check_contradictions`] to find pairwise conflicts between groups,
//!   plus an `overall_unsat` flag for the joint set.
//!
...
//! coefficient]` pairs; `constant` is added directly.
...
pub struct ProveReport {
    /// Whether the joint set of *all* constraints is unsatisfiable. `None`
    /// when the solver could not decide.
...
pub struct GroupResult {
    pub label: String,
    /// True when the group's own constraints are self-contradictory.
...
/// Build a [`ProveReport`] from parsed groups, running the self-unsat check
/// per group and the joint/pairwise check across groups.
